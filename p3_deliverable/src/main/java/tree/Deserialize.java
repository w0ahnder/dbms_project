...
public class Deserialize {
  ...
  public static void file_to_tree() {
    try {
      ...
      FileInputStream in =
          new FileInputStream("src/test/resources/samples-2/expected_indexes/Boats.E");
      ...
      HashMap<Integer, Set<Integer>> sizes =
          new HashMap<>(); // layer id, number of indexes on this level
      int level = 0; // means root

      for (int i = root_addr;
          i > num_leaves;
          i--) { // process each page; need to figure out how to distinguish levels
  ...
  public static Node deserializeIndex(ByteBuffer b, FileChannel fc, int page) {
    ...
    for (int i = 0;
        i < num_keys + 1;
        i++) { // get all the addresses, now follow all of the addresses to deserialize
