...
public class SelectPlan {
  ...
  public void plan(Expression expression) {
    ...
    for (Expression exp : expressions) {
      ...
      boolean tableHasIndex =
          DBCatalog.getInstance()
                  .getAvailableIndexColumn(DBCatalog.getInstance().getTableName(tableName))
              != null;
      boolean colHasIndex =
  ...
  public Operator optimalPlan() throws FileNotFoundException {
    ...
    for (String col : colMinMax.keySet()) {
      ...
      Tuple index_info =
          DBCatalog.getInstance()
              .getIndexInfo(DBCatalog.getInstance().getTableName(tableName), col);
  ...
  public ScanOperator createOP(Expression unindexed, String col) throws FileNotFoundException {
    ...
    Integer ind =
        DBCatalog.getInstance().colIndex(DBCatalog.getInstance().getTableName(tableName), col);
    boolean clustered =
        DBCatalog.getInstance()
                .getClustOrd(DBCatalog.getInstance().getTableName(tableName), col)
                .getElementAtIndex(0)
            == 1;
