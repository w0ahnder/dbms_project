...
public class IndexScanOperator2 extends ScanOperator {
  ...
  public IndexScanOperator2(
    ...
    reader = new TupleReader(DBCatalog.getInstance().getFileForTable(table)); // to read table file
    first_read = true;
    ...
    try {
      if (!isClustered) { // if it's not a clustered index
        ...
        if (first_read) { // checks if this is the first time calling getNextTuple
          ...
          reader.reset(
              rid[0], rid[1]); // fixes the tupleReader pointer to a particular position in the file
  ...
  private class PageItem {
    ...
    public String toString() {
      ...
      if (this.isLeaf) {
        build
            .append("Leaf: pageNumber: " + page + " currentLeafRidPosition:")
        ...
        build
            .append("Index: pageNumber: " + page + " numberOfIndexKeys:")
      ...
      try {
        if (page_num == 0) {
          ...
          while (buff.hasRemaining()) {
            int val = buff.getInt();
            // System.out.println("in while loop page num: " + page_num + " pagevalues: " +
            // pageValues);
            pageValues.add(val);
